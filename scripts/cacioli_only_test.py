"""
Test: correlazione con calibrazione SOLO Cacioli (niente PC).
Replica esatta di phase2 ma con solo Cacioli — stesso SBERT, stessa sigma, anchors = mean GT Cacioli.
Risponde a: il +0.30 su BLEU_out viene dal corpus PC o c'è segnale in Cacioli?
"""
...
CACHE = BASE / "scripts/cacioli_llm_cache"
RES   = BASE / "scripts/cacioli_results.json"
...
# ── Stessa struttura di phase2: slot-major, 6*N righe ────────────────────────
...
# ── Stesso modello SBERT di phase2 ───────────────────────────────────────────
...
# ── Sigma: mediana pairwise — identica a phase2 ───────────────────────────────
...
# ── Anchors: media GT per slot su Cacioli (equivalente a PC GT mean in phase2) ─
...
print(f"Anchors in : {[f'{a:.4f}' for a in anchors_in]}")
print(f"Anchors out: {[f'{a:.4f}' for a in anchors_out]}")
...
def h_spec(emb_in6, emb_out6):
    ...
    ev = np.abs(np.linalg.eigvals(laplacian(pts_out, sigma_out) @ laplacian(pts_in, sigma_in)).real)
    ev = ev[ev > 1e-12]; ev /= ev.sum()
    return float(-np.sum(ev * np.log(ev + 1e-12)))

# Per-clip: estrai embedding slot 0-5
print("Computing H_spectral ...")
...
print("\n" + "="*60)
print("  Cacioli-ONLY (SVM Cacioli, anchors GT Cacioli, multilingual)")
print("="*60)
...
print()
print("  Union-calibrated (PC+Cacioli) per confronto:")
print("  WER_in  : r=+0.1562  p=6.43e-02")
print("  WER_out : r=+0.2216  p=8.27e-03")
print("  BLEU_out: r=+0.2951  p=3.81e-04")
print("  E_sem   : r=+0.3279  p=7.23e-05")
